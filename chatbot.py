...
def process_user_query(query):
    ...
    elif "precio" in query.lower() or "cuánto es" in query.lower():
        return inform_total_price()
...
def consult_menu_csv(query):
    ...
    for category, items in st.session_state.menu.items():
        response += f"{category}:\n"
        for item in items:
            response += f"- {item['Producto']}: ${item['Precio']}\n"
        response += "\n"
    return response
...
def get_nutritional_info(query):
    ...
    for category, items in st.session_state.menu.items():
        for item in items:
            if item['Producto'].lower() == item_name:
                return f"Información nutricional para {item['Producto']}:\n" \
                       f"Calorías: {item['Calorías']} kcal\n" \
                       f"Grasas: {item['Grasas']}g\n" \
                       f"Proteínas: {item['Proteínas']}g\n" \
                       f"Carbohidratos: {item['Carbohidratos']}g"
    return "Lo siento, no pude encontrar información nutricional para ese producto."
    ...
    for category, items in st.session_state.menu.items():
        for item in items:
            if item['Producto'].lower() == item_name:
                ...
                return f"Has agregado {item['Producto']} a tu pedido. ¿Deseas algo más?"
...
def confirm_order():
    if st.session_state.order_in_progress and st.session_state.current_order:
        ...
        total_price = sum(float(item['Precio']) for item in st.session_state.current_order)
        st.session_state.order_in_progress = False
        st.session_state.current_order = []
...
def inform_total_price():
    if st.session_state.current_order:
        total_price = sum(float(item['Precio']) for item in st.session_state.current_order)
        return f"El precio total de tu pedido actual es ${total_price:.2f}."
    else:
        return "No tienes productos en tu pedido actual."

def save_order_to_csv(order):
    filename = 'orders.csv'
    file_exists = os.path.isfile(filename)
    with open(filename, 'a', newline='', encoding='utf-8') as csvfile:
        fieldnames = ['Fecha', 'Producto', 'Precio']
